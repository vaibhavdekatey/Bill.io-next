...
export default function NewInvoice() {
  ...
  useEffect(() => {
    const fetchInvoiceNumber = async () => {
      try {
        const res = await api.get("/invoices?action=generate");
  ...
  useEffect(() => {
    ...
    const fetchInvoice = async () => {
      try {
        const res = await api.get(`/invoices/${editId}`);
  ...
  const handleSubmit = async (status: "DRAFT" | "SENT") => {
    try {
      ...
      console.error(
        isEditMode ? "Error updating invoice:" : "Error creating new invoice:",
        error,
      );
    } finally {
      setSubmitting(false);
      ...
      <div className="sticky top-0 z-10 px-4 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-black">
        ...
        <div className="flex flex-wrap gap-2 sm:ml-auto w-full sm:w-auto">
          ...
          <PillButton
            ...
            arrow={true}
            title={isEditMode ? "Update Invoice" : "Send Invoice"}
            disabled={submitting}
            varient=""
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-x-4 gap-y-6 items-start px-4 pb-4">
        ...
        <div className="  flex flex-col gap-4 sticky top-[73px]">
          <Panel title="Invoice details">
            <MetaField label="Invoice number">
              <input
